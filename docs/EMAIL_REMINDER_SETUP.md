...
1. 「リマインダーを追加」セクションで以下を入力：
   ...
   - **通知チャネル**: **メール** を選択
   - **送信先メールアドレス（任意）**: カスタムメールアドレスを指定可能
     - 空欄の場合は、ユーザーのアカウントに登録されているメールアドレスに送信されます
     - 別のメールアドレスに送信したい場合は、ここに入力してください
   - **通知日時**: 送信したい日時を指定
2. 「リマインダーを登録」ボタンをクリック

**使用例:**
- 自分のメールアドレス以外に送信したい場合（チームメンバー、上司など）
- テスト用のメールアドレスで動作確認したい場合
